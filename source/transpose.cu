...
__global__ void matrixTransposeNaive(const float* const a, float* const b)
{
    ...
    int index_out = j + sizeY * i;  // Compute output index (j,i) in matrix B = transpose(A)
...
__global__ void matrixTransposeShared(const float* const a, float* const b)
{
    ...
    int ti = threadIdx.y + by;     // Global output x index - remember the transpose
    int tj = threadIdx.x + bx;     // Global output y index - remember the transpose

    //Copy datafrom input to shared memory
	mat[threadIdx.y][threadIdx.x] = a[i + sizeX *j];
	__syncthreads();
    //Copy data from shared memory to global memory bx
	b[ti + sizeX * tj] = mat[threadIdx.y][threadIdx.x];
}
...
#define CPU_TRANSPOSE
    ...
    cout << "***Shared Memory Transpose with Dynamic Shared Memory***" << endl;
    {
        ...
        // Launch the GPU __kernel
		int sharedMemoryPerBlockInBytes = dims.dimBlock.x * dims.dimBlock.y * sizeof(float); // Compute This
        // Call kernel - 
		matrixTransposeDynamicShared<<<dims.dimGrid, dims.dimBlock, 
			sharedMemoryPerBlockInBytes>>>(d_a, d_b);
